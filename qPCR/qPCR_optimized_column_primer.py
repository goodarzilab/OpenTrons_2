...
def map_tube_to_well(dataframe, plate_384, temp_labware, primer_tube_positions, sample_tube_positions, p20_single, protocol):
    for index, col in dataframe.items():
        ...
        if len(primers_in_columns) != 1:
            protocol.comment('Multiple primers in the column')
            ...
            for row, location in enumerate(col):
                ...
                dest_well = plate_384.wells()[index * len(dataframe.rows) + row]
            ...
            protocol.comment('Only one primer in the column')
            ## If there is only one primer in the column, then we can distribute the primer to all wells in the column
            primer = primers_in_columns.pop()
            primer_source_well = primer_tube_positions.loc[primer, 'Well']
            primer_source_well = temp_labware.wells_by_name()[primer_source_well]

            p20_single.distribute(2.5, primer_source_well, [plate_384.wells()[index * len(dataframe.index) + row] for row, location in enumerate(col)])
            ...
            for row, location in enumerate(col):
                ...
                #print('primer {} sample {}'.format(primer, sample))
                sample_source_well = sample_tube_positions.loc[sample, 'Well']
                dest_well = plate_384.wells()[index * len(dataframe.index) + row]
                sample_source_well = temp_labware.wells_by_name()[sample_source_well]
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Set temp to 4C
    ## Change this to 4C during the actual run
    temp_mod.set_temperature(celsius=22)
    temp_mod.status  # 'holding at target'

    # Pipettes
    p20_single = protocol.load_instrument('p20_single_gen2', 'right', tip_racks=[tiprack_20ul_1, tiprack_20ul_2, tiprack_20ul_3])
    #p300_single = protocol.load_instrument('p300_single_gen2', 'left', tip_racks=[tiprack_200ul_1])
    ...
    # Transfer components
    map_tube_to_well(transfer_data, plate_384, temp_labware, primer_tube_positions, sample_tube_positions, p20_single, protocol)
