...
def map_tube_to_well(dataframe, p384, temp_labware, primer_tube_positions, sample_tube_positions, p20_single, p300_single, protocol):
    for index, col in dataframe.items():
        ...
        if len(primers_in_columns) != 1:
            ...
            p300_single.distribute(2.5, primer_source_well, [p384.wells()[index * len(dataframe.index) + row] for row, location in enumerate(col)])
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Transfer components
    map_tube_to_well(transfer_data, p384, temp_labware, primer_tube_positions, sample_tube_positions, p20_single, p300_single, protocol)
